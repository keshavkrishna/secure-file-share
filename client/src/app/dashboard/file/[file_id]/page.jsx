...
import { downloadFile, fetchFileForView, fetchFileDetails, shareFile, deleteFile, revokeFileAccess} from "../../../lib/encrypt";
...
export default function FileDetailPage({ params }) {
  ...
  const handleGenerateLink = async () => {
    setError("");
    try {
      const details = await generateSharableLink(param.file_id);
  ...
  const  handleDeleteLink = async (token) => {
    try{
        const message = await deleteSharableLink(param.file_id, token);
  ...
  const handleViewFile = async () => {
    setError("");
    try {
      const { blob, name } = await fetchFileForView(param.file_id);
  ...
  const handleDownloadFile = async () => {
    try {
      await downloadFile(param.file_id);
    } catch (err) {
  ...
  const handleDeleteFile = async () => {
    try {
      await deleteFile(param.file_id);
  ...
  const handleShareClick = async () => {
    ...
    if(username)
    {
        const response = await shareFile(param.file_id,username, canDownload);
  ...
  const handleTogglePermission = async (username, permission, value) => {
    try {
      ...
      await shareFile(param.file_id, username,  value);
  ...
  const handleRevokePermission = async (username) => {
    try {
      await revokeFileAccess(param.file_id, username);
      setFileDetails((prevDetails) => ({
        ...prevDetails,
                ...
                <div className="mt-6">
                    ...
                    <ul className="space-y-4">
                        ...
                        >
                            ...
                            <span className="font-medium">Link:</span> {`http://localhost:3000/dashboard/file/link/${link.token}`}
                            </p>
                            <p className="text-sm text-gray-600">
